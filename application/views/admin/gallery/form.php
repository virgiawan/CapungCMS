...
<div id="main">
	<?php if(!$is_edit):?>
		<h3>Add Gallery <a class="btn_add" href="<?php echo  site_url('admin/gallery') ?>">[ Cancel ]</a> </h3>
		<form action="<?php echo site_url('admin/gallery/add')?>" method="POST" enctype="multipart/form-data">
	<?php else:?>
		<h3>Edit Gallery <a class="btn_add" href="<?php echo  site_url('admin/gallery') ?>">[ Cancel ]</a> </h3>		
		<form action="<?php echo site_url('admin/gallery/edit/'.$id)?>" method="POST" enctype="multipart/form-data">
	<?php endif;?>
	<fieldset>
		...
		<div class="field <?php echo form_error('thumbnail')?'div-error':''; ?>">
			...
			<?php if($is_edit && !empty($post->thumbnail)):?>
				<p>
					...
					[ <a href="<?php echo site_url('admin/gallery/remove_upload/'.$post->id.'/thumbnail');?>" class="link_manage" >remove</a> ]
				</p>
			<?php endif;?>
		</div>
		<div class="field <?php echo form_error('image')?'div-error':''; ?>">
			...
			<?php if($is_edit && !empty($post->filename)):?>
				<p>
					...
					[ <a href="<?php echo site_url('admin/gallery/remove_upload/'.$post->id.'/filename'); ?>" class="link_manage" >remove</a> ]
				</p>
			<?php endif;?>
		</div>
