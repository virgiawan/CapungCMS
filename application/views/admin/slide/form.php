...
<div id="main">
	<?php if(!$is_edit):?>
		<h3>Add Slide <a class="btn_add" href="<?php echo  site_url('admin/slide') ?>">[ Cancel ]</a> </h3>
		<form action="<?php echo site_url('admin/slide/add')?>" method="POST" enctype="multipart/form-data">
	<?php else:?>
		<h3>Edit Slide <a class="btn_add" href="<?php echo  site_url('admin/slide') ?>">[ Cancel ]</a> </h3>		
		<form action="<?php echo site_url('admin/slide/edit/'.$id)?>" method="POST" enctype="multipart/form-data">
	<?php endif;?>
	<fieldset>
		...
		<div class="field <?php echo form_error('image')?'div-error':''; ?>">
			...
			<?php if($is_edit && !empty($post->filename)):?>
				<p>
					...
					[ <a href="<?php echo site_url('admin/slide/remove_upload/'.$post->id.'/filename'); ?>" class="link_manage" >remove</a> ]
				</p>
			<?php endif;?>
		</div>
